...
char choisirManiereDeplacement(int lesX[TAILLE], int lesY[TAILLE], tPlateau t, int xDest, int yDest);
void choisirManiereDeplacementProgressionPart(int lesX[TAILLE], int lesY[TAILLE], tPlateau t, bool *est_arrive, char direction);

char calculerDirectionVertical(int lesX[TAILLE], int lesY[TAILLE], int xPomme, int yPomme, tPlateau t);
char calculerDirectionHorizontal(int lesX[TAILLE], int lesY[TAILLE], int xPomme, int yPomme, tPlateau t);

char calculerDirectionElsePart(int lesX[TAILLE], int lesY[TAILLE], tPlateau t);
...
int lesPavesX[NOMBRE_PAVE] = {3, 74, 3, 74, 38, 38};
int lesPavesY[NOMBRE_PAVE] = {3, 3, 34, 34, 21, 15};
...
int main(){
	...
	char deplacementWay = choisirManiereDeplacement(lesX, lesY, lePlateau, xDest, yDest);
	...
	do{
		if (deplacementWay == VERTICAL){
			direction = calculerDirectionVertical(lesX, lesY, xDest, yDest, lePlateau);
		}
		
		else {
			direction = calculerDirectionHorizontal(lesX, lesY, xDest, yDest, lePlateau);
		}
		...
		if (tunnel){
			...
			tunnel = false;
			deplacementWay = choisirManiereDeplacement(lesX, lesY, lePlateau, xDest, yDest);
		}
...
char choisirManiereDeplacement(int lesX[TAILLE], int lesY[TAILLE], tPlateau t, int xDest, int yDest){
	...
	*/
	...
	while (arriveVertical == false && arriveHorizontal == false){
		...
		directionHorizontal = calculerDirectionHorizontal(lesXHorizontaux, lesYHorizontaux, xDest, yDest, t);
		directionVertical = calculerDirectionVertical(lesXVerticaux, lesYVerticaux, xDest, yDest, t);
...
char calculerDirectionVertical(int lesX[TAILLE], int lesY[TAILLE], int xDest, int yDest, tPlateau t){
	/*
	...
	*/
	...
	//Si aucune direction directe n'est possible, on teste les directions restantes jusqu'à ce que l'une soit valide
		...
		direction = calculerDirectionElsePart(lesX, lesY, t);
	}
	return direction;
}


char calculerDirectionHorizontal(int lesX[TAILLE], int lesY[TAILLE], int xDest, int yDest, tPlateau t){
	/*
	...
	*/
	...
	if (lesX[0] < xDest && peutAllerDansDirection(DROITE, lesX, lesY, t)){
		direction = DROITE; //Si le serpent peut aller à droite, choisir la direction DROITE
	}
	...
	else if (lesY[0] < yDest && peutAllerDansDirection(BAS, lesX, lesY, t)){
		direction = BAS; //Si le serpent peut aller en bas, choisir la direction BAS
	}
...
bool peutAllerDansDirection(char direction, int lesX[TAILLE], int lesY[TAILLE], tPlateau t){
	...
	if (direction == BAS){
		return (t[lesX[0]][lesY[0] + 1] != BORDURE && !snakeCollision(BAS, lesX, lesY) && t[lesX[0]][lesY[0] + 1] != PAVE);
	}

	//Vérifie si la case au-dessus de la tête du serpent n'est pas une BORDURE et s'il n'y a pas de collision.
	else if (direction == HAUT){
		return (t[lesX[0]][lesY[0] - 1] != BORDURE && !snakeCollision(HAUT, lesX, lesY) && t[lesX[0]][lesY[0] - 1] != PAVE);
	}

	//Vérifie si la case à droite de la tête du serpent n'est pas une BORDURE et s'il n'y a pas de collision.
	else if (direction == DROITE){
		return (t[lesX[0] + 1][lesY[0]] != BORDURE && !snakeCollision(DROITE, lesX, lesY) && t[lesX[0] + 1][lesY[0]] != PAVE);
	}

	//Vérifie si la case à gauche de la tête du serpent n'est pas une BORDURE et s'il n'y a pas de collision.
	else if (direction == GAUCHE){
		return (t[lesX[0] - 1][lesY[0]] != BORDURE && !snakeCollision(GAUCHE, lesX, lesY) && t[lesX[0] - 1][lesY[0]] != PAVE);
	}
